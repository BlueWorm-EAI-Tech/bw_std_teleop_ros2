...
manager::VrStreamConfig BwTeleop::load_stream_config()
{
  ...
  const std::vector<double> left_reset_position = declare_array(
    "left_reset_position", {0.27, 0.25, -0.15}, 3U);
  ...
  const std::vector<double> right_reset_position = declare_array(
    "right_reset_position", {0.27, -0.25, -0.15}, 3U);
  ...
  config.lift_min = declare_parameter<double>("lift_min", -0.231);
  config.lift_max = declare_parameter<double>("lift_max", 0.231);
  config.lift_initial_position = declare_parameter<double>("lift_initial_position", 0.0);
  config.lift_jog_speed = declare_parameter<double>("lift_jog_speed", 0.10);
...
void BwTeleop::publish_commands()
{
  ...
  if (result.output.auxiliary_commands_valid) {
    ...
    lift_command.data = {result.output.lift_command};
